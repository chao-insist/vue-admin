  ...
  <el-row class="wrap">
    ...
    <el-col :span="24" class="wrap-breadcrum">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>学院专题</el-breadcrumb-item>
      </el-breadcrumb>
    </el-col>
    <el-col :span="24" class="wrap-main">
      ...
      <el-table :data="news" highlight-current-row @selection-change="selsChange" style="width:100%" v-loading="listLoading">
        ...
        <el-table-column prop="academyName" label="专题名称"></el-table-column>
      ...
      <el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false">
        <el-form :model="editForm" label-width="100px" ref="editForm" :rules="formRules">
          <el-form-item label="专题名称" prop="academyName">
            <el-input v-model="editForm.academyName" auto-complete="off"></el-input>
          </el-form-item>
          <el-form-item label="排序" prop="sort">
            <el-input-number v-model="editForm.sort" auto-complete="off" :min="1" :max="10"></el-input-number>
            <span style="color: orange;margin-left: 20px;">tip: 数字越小权重越高，注意区分相同专题的权重！</span>
          </el-form-item>
        </el-form>
      ...
      <el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false">
        <el-form :model="addForm" label-width="100px" ref="addForm" :rules="formRules">
          <el-form-item label="专题名称" prop="academyName">
            <el-input v-model="addForm.academyName" auto-complete="off"></el-input>
          </el-form-item>
          <el-form-item label="排序" prop="sort">
            <el-input-number v-model="addForm.sort" auto-complete="off" :min="1" :max="10"></el-input-number>
            <span style="color: orange;margin-left: 20px;">tip: 数字越小权重越高，注意区分相同专题的权重！</span>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
...
import {reqSchoolModuleList,reqAddSchoolModule,reqDeleteSchoolModule,reqEditSchoolModule,reqGetListByTitle} from 'api/api'
export default {
  name : 'SchoolModule',
  computed : {
  	//设置状态标签
  ...
  data () {
    return {
      ...
      formRules : {
        ...
        sort: [
        	{type: "number", required: true, message: "请对专题进行排序", trigger: "change"}
        ]
      }
    }
  },
  ...
  //获取列表
    getList () {
      if(!this.isQuery){
	      ...
	      reqSchoolModuleList(params).then(res=>{
        ...
        if(valid){
          this.$confirm("确认新增此条合作伙伴吗？","提示",{type:"warning"}).then(()=>{
            ...
            params.append("sort",this.addForm.sort)
						reqAddSchoolModule(params).then(res=>{
    ...
    delnew (index,row) {
      this.$confirm("确认删除该条记录吗？","提示",{type:"warning"}).then(()=>{
        ...
        reqDeleteSchoolModule(params).then(res=>{
            this.listLoading = false;
            this.$message({
              message : "删除成功",
              type : "success"
        ...
        if(valid){
          this.$confirm("确认提交吗","提示").then(()=>{
            ...
            params.append("sort",this.editForm.sort)
						reqEditSchoolModule(params).then(res=>{		
	            	let {code, msg} = res.data;
	            	if(code != 2000){
	            		this.$message({
	                  message:"提交成功",
