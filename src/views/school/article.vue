  ...
  <el-row class="wrap">
    ...
    <el-col :span="24" class="wrap-breadcrum">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>学院文章</el-breadcrumb-item>
      </el-breadcrumb>
    </el-col>
    <el-col :span="24" class="wrap-main">
      ...
      <el-table :data="news" highlight-current-row @selection-change="selsChange" style="width:100%" v-loading="listLoading">
        ...
        <el-table-column prop="academyName" label="所属专题" sortable></el-table-column>
        <el-table-column prop="views" label="阅读数" sortable></el-table-column>
        <el-table-column prop="updated" label="修改时间" :formatter="formatDate"></el-table-column>
      ...
      <!-- 编辑界面 -->
      <!--参数：标题，内容，图片，阅读数，所属专题，关键词，关键词描述-->
      <el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false">
        <el-form :model="editForm" label-width="100px" ref="editForm" :rules="formRules">
          ...
          <el-form-item prop="viewsOut" label="外部阅读数">
          	<el-input-number v-model="editForm.viewsOut" :min="1"></el-input-number>
          </el-form-item>
          <el-form-item prop="academyName" label="所属专题">
          	<el-select v-model="editForm.academyName" placeholder="请选择专题">
          		<el-option v-for="item in moduleList" :value="item"></el-option>
          	</el-select>
          </el-form-item>
      ...
      <el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false">
        <el-form :model="addForm" label-width="100px" ref="addForm" :rules="formRules">
         <el-form-item label="标题" prop="title">
          ...
          <el-form-item prop="academyName" label="所属专题">
          	<el-select v-model="addForm.academyName" placeholder="请选择专题">
          		<el-option v-for="item in moduleList" :value="item"></el-option>
          	</el-select>
          </el-form-item>
          <el-form-item prop="keywords" label="关键词">
...
import {reqSchoolArticleList,reqAddSchoolArticle,reqDeleteSchoolArticle,reqEditSchoolArticle,reqGetListByTitle,reqSelectModuleList,picUpload} from 'api/api'
...
//	      autoFloatEnabled: false,
  ...
  created() {
  	this.academyName = JSON.parse(sessionStorage.getItem("admin-user")).name;
  },
  methods : {
  	//获取编辑内容
  	getUeEditContent(){
  ...
  //获取列表
    getList () {
      if(!this.isQuery){
	      ...
	      reqSchoolArticleList(params).then(res=>{
	      ...
	      reqSelectModuleList(params).then(res=>{
	      	console.log(res.data.data)
	      	let data = res.data.data;
	      	this.moduleList = [];
	      	for(var i = 0;i < data.length;i++){
	      		this.moduleList.push(data[i].academyName);
	      	}
      ...
      this.$refs.addForm.validate((valid)=>{    //验证表单
        if(valid){
          this.$confirm("确认新增此条合作伙伴吗？","提示",{type:"warning"}).then(()=>{
            ...
            let params = new URLSearchParams();
            params.append("compile",this.academyName);		//昵称
            ...
            params.append("academyName",this.addForm.academyName)
            reqAddSchoolArticle(params).then(res=>{
    ...
    delnew (index,row) {
      this.$confirm("确认删除该条记录吗？","提示",{type:"warning"}).then(()=>{
        ...
        reqDeleteSchoolArticle(params).then(res=>{
            this.listLoading = false;
            this.$message({
              message : "删除成功",
              type : "success"
      ...
      this.$refs.editForm.validate((valid)=>{
        if(valid){
          this.$confirm("确认提交吗","提示").then(()=>{
            ...
            params.append("academyName",this.editForm.academyName)
            reqEditSchoolArticle(params).then(res=>{
            	let {code, msg} = res.data;
            	if(code != 2000){
            		this.$message({
                  message:"提交成功",
