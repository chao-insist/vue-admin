	...
	<el-row class="wrap">
		...
		<el-col :span="24" class="wrap-breadcrum">
			<el-breadcrumb separator="/">
				...
				<el-breadcrumb-item>币交所</el-breadcrumb-item>
			</el-breadcrumb>
		</el-col>
		...
		<el-col class="wrap-main">
			...
			<!--列表-->
			<!--参数：ID，名称，创建时间，创始人，信用等级，发表时间，状态-->
			 <el-table :data="bitePlaceList" highlight-current-row @selection-change="selsChange" style="width:100%" v-loading="listLoading">
		        ...
		        <el-table-column prop="level" label="信用等级" sortable></el-table-column>
		      ...
		      <!--编辑界面-->
		      <!--参数：标题，内容，图片，中文名称，创始时间，创始人，交易币种，所在地，星级，关键词，关键描述-->
		      <el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false">
		      	<el-form :model="editForm" ref="editForm" :rules="formRules" label-width="100px">
		      		...
		      		<el-form-item prop="dealCurrency" label="交易币种">
		      			<!--<el-select v-model="dealCurrency.value" placeholder="请选择">
		      				<el-option v-for="item in dealCurrency.options" :key="item" :value="item"></el-option>
		      			</el-select>-->
		      			<el-input v-model="editForm.dealCurrency"></el-input>
		      		</el-form-item>
		      ...
		      <el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false">
		      	<el-form :model="addForm" ref="addForm" :rules="formRules" label-width="100px">
		      		...
		      		<el-form-item prop="dealCurrency" label="交易币种">
		      			...
		      			<el-input v-model="addForm.dealCurrency"></el-input>
		      		</el-form-item>
		      		<el-form-item label="地域" prop="location">
...
<script>
	...
	import {reqGetBiteList,reqAddBitePlace,reqDeleteBite,reqEditBite,reqGetListByTitle,picUpload} from 'api/api'
	export default {
		computed : {
			//设置状态标签
			statusTag (val) {
			...
			getList () {
				if(!this.isQuery){
					...
					reqGetBiteList(params).then(res=>{
						...
						this.bitePlaceList = res.data.list;
						console.log(this.bitePlaceList);
					})
				}else{
					this.getTitleList();
				}
		    	...
		    	this.$refs.addForm.validate(valid=>{
		    		if(valid){
		    			this.$confirm("确认增加一条币信息吗","提示").then(()=>{
		    				...
		    				params.append('chineseName',"default");
			    			...
			    			params.append('dealCurrency',this.addForm.dealCurrency);
			    			...
			    			reqAddBitePlace(params).then(res=>{
			    				if(res.code != 2000){
				    				this.$message({
				    					message:"添加成功",
				    					type:"success"
...
//			    			params.append('englishName',this.editForm.englishName);
			    			...
			    			params.append('dealCurrency',this.editForm.dealCurrency);
