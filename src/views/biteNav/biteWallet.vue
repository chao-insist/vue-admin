	...
	<el-row class="wrap">
		...
		<el-col :span="24" class="wrap-breadcrum">
			<el-breadcrumb separator="/">
				...
				<el-breadcrumb-item>币钱包</el-breadcrumb-item>
			</el-breadcrumb>
		</el-col>
			 ...
			 <el-table :data="biteWalletList" highlight-current-row @selection-change="selsChange" style="width:100%" v-loading="listLoading">
		      ...
		      <el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false">
		      	<el-form :model="editForm" ref="editForm" :rules="formRules" label-width="100px">
		      		...
		      		<el-form-item prop="chineseName" label="钱包名称">
		              ...
		              <el-input v-model="editForm.supportCurrency" auto-complete="off"></el-input>
		            </el-form-item>
		      ...
		      <el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false">
		      	<el-form :model="addForm" ref="addForm" :rules="formRules" label-width="100px">
		      		...
		      		<!--supportCurrency-->
		      		<el-form-item label="支持币种" prop="supportCurrency">
		              <!--<el-checkbox-group v-model="addForm.supportCurrency" :min="1" :max="2">
		                <el-checkbox-button v-for="item in this.supportCurrencyBox" :label="item" :key="item">{{item}}</el-checkbox-button>
		              </el-checkbox-group>-->
...
<script>
	...
	import {reqGetBiteList,reqAddBiteWallet,reqDeleteBite,reqEditBite,reqGetListByTitle,picUpload} from 'api/api'
	export default {
		computed : {
			//设置状态标签
			statusTag (val) {
		    	...
		    	this.$refs.addForm.validate(valid=>{
		    		if(valid){
		    			this.$confirm("确认增加一条币信息吗","提示").then(()=>{
			    			...
			    			reqAddBiteWallet(params).then(res=>{
			    				if(res.code != 2000){
				    				this.$message({
				    					message:"添加成功",
				    					type:"success"
